...
//	m_bShowDropdownArrowWhenVertical = FALSE;
	...
	m_strSubKey.Format( _T("Software\\%s\\Settings"), pApp->m_pszAppName);
...
CToolBarEx::~CToolBarEx()
{
	...
	if( m_pControls ) 
	{
		for ( POSITION pos = m_pControls->GetHeadPosition(); pos != NULL; )
		{
...
BOOL CToolBarEx::ShowContextMenu(CPoint point)
{
	...
	{
		// Control Bars array
		CArray<CControlBar*,CControlBar*> arBars;
		...
		if (pFrame && pFrame->IsKindOf(RUNTIME_CLASS(CFrameWnd)) )
		{
			...
			while (pos != NULL)
			{
				...
				if (pBar && pBar->GetStyle()&CCS_ADJUSTABLE && 
					pBar->IsKindOf(RUNTIME_CLASS(CToolBar))) // Put any other type

				{
					// Add Toolbar to the array
					i++; arBars.Add(pBar);
		...
		int nResult = popupMenu.TrackPopupMenu(TPM_LEFTALIGN|TPM_LEFTBUTTON|TPM_VERTICAL|TPM_RETURNCMD,
						pt.x, pt.y, this);
		...
		switch(nResult)
		{
			...
			{
				ASSERT_VALID(pFrame);
				if (pFrame && arBars[nResult-1] &&
					pFrame->IsKindOf(RUNTIME_CLASS(CFrameWnd)) )
				{	
					BOOL bVisible = ((arBars[nResult-1]->GetStyle() & WS_VISIBLE) != 0);

					pFrame->ShowControlBar(arBars[nResult-1], !bVisible, FALSE);
					pFrame->RecalcLayout();
				}
				break;
			}
