...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    ...
    m_nEditMode          = GVEM_NORMAL_GRID;
    m_bMouseLBPressed    = FALSE;
...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
    ...
    TRY {
        ...
        m_arColProps.SetSize(m_nCols);      // initialize column props
    }
    ...
    for (int i = 0; i < m_nCols; i++) {
		m_arColProps[i].width = m_nDefCellWidth;
		m_arColProps[i].edit = GVCE_EDIT;
		m_arColProps[i].mapVal = NULL;
	}
...
LRESULT CGridCtrl::SendMessageToParent(int nMessage, int nRow, int nCol, 
									   LPCTSTR szText)
{
    if (!IsWindow(m_hWnd))
        return 0;
...
BEGIN_MESSAGE_MAP(CGridCtrl, CWnd)
	...
	ON_NOTIFY(GVN_EDITSELENDOK, IDC_INPLACE_CONTROL, OnSelOkInPlaceList)
    ON_NOTIFY(GVN_CHECKCELLDATA, IDC_INPLACE_CONTROL, OnCheckInPlaceData)
...
void CGridCtrl::OnSetFocus(CWnd* pOldWnd) 
{
	...
	// Set focus to InPlaceEdit, if exists
	if (IsWindow(m_hCurEdit) && m_pCurEdit != NULL)
		m_pCurEdit->SetFocus();
}
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
    if (!IsValid(m_idCurrentCell)) 
    {
		...
		CWnd* own = GetOwner();
		if (own != NULL && nChar == VK_TAB)
		{
			CWnd* wItem;
            if (IsSHIFTpressed())
				wItem = own->GetNextDlgTabItem(this, TRUE);
			else
				wItem = own->GetNextDlgTabItem(this, FALSE);
			if (wItem != NULL) wItem->SetFocus();
		}
        return;
    }
    ...
    switch (nChar)
    {
        case VK_DELETE: 
            ...
            {
				SendMessageToParent(GVN_BEGINLABELEDIT, 
                ...
                SetModified(TRUE, m_idCurrentCell.row, m_idCurrentCell.col);
 				SendMessageToParent(GVN_ENDLABELEDIT, 
					m_idCurrentCell.row, m_idCurrentCell.col, "");
	...
	// Если следующая ячейка не равна текущей
    if (next != m_idCurrentCell) 
    {
        // While moving with the Cursorkeys 
		// the current ROW/CELL will get selected
        // OR Selection will get expanded when SHIFT is pressed
        // Cut n paste from OnLButtonDown - Franco Bez
		if (!m_bMouseLBPressed ||
			 m_MouseMode != MOUSE_SIZING_ROW &&
			 m_MouseMode != MOUSE_SIZING_COL)
		{
			if (SetFocusCell(next))
			{
				int nMouseMode = m_MouseMode;
				m_PrevSelectedCellMap.RemoveAll();
				m_MouseMode = m_bListMode? MOUSE_SELECT_ROW : MOUSE_SELECT_CELLS;
				if (!IsSHIFTpressed() || nChar == VK_TAB)
					m_SelectionStartCell = next;
				OnSelecting(next);
				m_MouseMode = nMouseMode;
			}
		}
...
void CGridCtrl::OnSelOkInPlaceList(NMHDR* pNMHDR, LRESULT* pResult) 
{    
    GV_DISPINFO *pgvDispInfo = (GV_DISPINFO *)pNMHDR;
    GV_ITEM     *pgvItem = &pgvDispInfo->item;

	// Send message to parent
    SendMessageToParent(GVN_EDITSELENDOK, 
		pgvItem->row, pgvItem->col, pgvItem->szText);

    *pResult = 0;
}

void CGridCtrl::OnCheckInPlaceData(NMHDR* pNMHDR, LRESULT* pResult) 
{    
    GV_DISPINFO *pgvDispInfo = (GV_DISPINFO *)pNMHDR;
    GV_ITEM     *pgvItem = &pgvDispInfo->item;
	...
	// Send Message for Checking cell data
    if (SendMessageToParent(GVN_CHECKCELLDATA, 
		pgvItem->row, pgvItem->col, pgvItem->szText) != 0)
	{
		*pResult = 1;
		return;
	}
...
void CGridCtrl::OnHScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	...
	// Show/Hide InPlaceEdit, if exists
    if (IsWindow(m_hCurEdit))
	{
		if (IsCellVisible(m_idCurrentCell))
		{
			CRect  cellRect;
			GetCellRect(m_idCurrentCell, cellRect);
			if (m_pCurEdit->IsKindOf(RUNTIME_CLASS(CInPlaceEdit)))
			{
				::SetWindowPos(m_hCurEdit, HWND_TOP, 
			...
			{
				::SetWindowPos(m_hCurEdit, HWND_TOP, 
					cellRect.left, cellRect.top, 0, 0, 
					SWP_SHOWWINDOW|SWP_NOSIZE);
			}
		}
		else
		{
			::ShowWindow(m_hCurEdit, SW_HIDE);
		}
		InvalidateRect(rect);
	}
}
...
BOOL CGridCtrl::SetFocusCell(int nRow, int nCol, BOOL bNotify)
{
    return SetFocusCell(CCellID(nRow, nCol), bNotify);
}

BOOL CGridCtrl::SetFocusCell(CCellID cell, BOOL bNotify)
{
	...
	// Посылка и проверка уведомлений
    if (IsValid(idPrev) && bNotify) 
    {
        if (SendMessageToParent(GVN_SELCHANGING, 
			idPrev.row, idPrev.col, "") != 0)
			return FALSE;

        if (idPrev.row != cell.row)
			if (SendMessageToParent(GVN_ROWCHANGING, 
				idPrev.row, idPrev.col, "") != 0)
				return FALSE;
	}
    ...
    if (IsValid(m_idCurrentCell)) {
		...
		if (bNotify)
		{
			SendMessageToParent(GVN_SELCHANGED, 
			m_idCurrentCell.row, m_idCurrentCell.col, "");

			if (idPrev.row != m_idCurrentCell.row)
				SendMessageToParent(GVN_ROWCHANGED, 
				m_idCurrentCell.row, m_idCurrentCell.col, "");
		}
    }
...
void CGridCtrl::CutSelectedText()
{
    ...
    for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
    {
        ...
        if (pCell)
        {
		    SendMessageToParent(GVN_BEGINLABELEDIT, cell.row, cell.col, "");
            EmptyCell(pCell, cell.row, cell.col);
            SetModified(TRUE, cell.row, cell.col);
		    SendMessageToParent(GVN_ENDLABELEDIT, cell.row, cell.col, "");
        }
    }
    Invalidate();
}
...
{        
    ...
    if (pSource) 
    {    
        SendMessageToParent(GVN_BEGINDRAG, 
			GetSelectedCellRange().GetTopLeft().row,
...
void CGridCtrl::ResetSelectedRange(BOOL bNotify)
{
    SetSelectedRange(-1,-1,-1,-1);
    SetFocusCell(-1, -1, bNotify);
}
...
void CGridCtrl::ResizeEditControl()
{
	// Resizing InPlaceEdit, if exists
	if (IsWindow(m_hCurEdit))
	{
		CRect rect;
		GetCellRect(m_idCurrentCell, rect);
		if (m_pCurEdit->IsKindOf(RUNTIME_CLASS(CInPlaceEdit)))
		{
			::SetWindowPos(m_hCurEdit, 0, 
				rect.left, rect.top, 
		...
		{
			RECT rcEdit;
			::GetWindowRect(m_hCurEdit, &rcEdit);
			::SetWindowPos(m_hCurEdit, 0, 
				rect.left, rect.top, 
				rect.right - rect.left, 
		...
		if (m_pCurEdit != NULL) m_pCurEdit->SetFocus();
	}
}
...
void CGridCtrl::ResetScrollBars()
{
    ...
    CRect VisibleRect(GetFixedColumnWidth(), 
		GetFixedRowHeight(), rect.right, rect.bottom);
    CRect VirtualRect(GetFixedColumnWidth(), 
		GetFixedRowHeight(), GetVirtualWidth(), GetVirtualHeight());
    ...
    if (VisibleRect.Height() < VirtualRect.Height())
		...
		//+ GetRowHeight(visibleCells.GetTopLeft().row);
    else
        m_nVScrollMax = 0;

    if (VisibleRect.Width() < VirtualRect.Width())
        m_nHScrollMax = VirtualRect.Width();
		//+ GetColumnWidth(visibleCells.GetTopLeft().col);
    else
        m_nHScrollMax = 0;
...
BOOL CGridCtrl::SetColumnCount(int nCols)
{
    ...
    if (addedCols < 0) {
		...
		// Remove Map of Values for each column
		for (int col = nCols; col < GetColumnCount(); col++)
			if (m_arColProps[col].mapVal != NULL)
			{
				delete m_arColProps[col].mapVal;
				m_arColProps[col].mapVal = NULL;
			}
    }

    // Change the number of columns.
    m_nCols = nCols;
    m_arColProps.SetSize(nCols);
    ...
    if (addedCols > 0)
    {
        ...
        int col, startCol = nCols - addedCols;
        for (col = startCol; col < GetColumnCount(); col++) {
            m_arColProps[col].width = m_nDefCellWidth;
			m_arColProps[col].style = GVCE_EDIT;
			m_arColProps[col].mapVal = NULL;
		}
                            ...
                            int nColumn  /* = -1 */)
{
    ...
    {
		GV_COLUMN	strCol;
        m_arColProps.InsertAt(nColumn, strCol);
        for (int row = 0; row < m_nRows; row++) 
        {
            GRID_ROW* pRow = m_RowData[row];
            if (!pRow) return -1;
    ...
    m_arColProps[nColumn].width = GetTextExtent(strHeading).cx;
	// initialize column edit control
	m_arColProps[nColumn].edit = GVCE_EDIT;
	m_arColProps[nColumn].mapVal = NULL;
...
BOOL CGridCtrl::DeleteColumn(int nColumn)
{
	...
	if (m_arColProps[nColumn].mapVal != NULL)
	{
		delete m_arColProps[nColumn].mapVal;
		m_arColProps[nColumn].mapVal = NULL;
	}
    m_arColProps.RemoveAt(nColumn);
    m_nCols--;
...
BOOL CGridCtrl::DeleteRow(int nRow, BOOL bNotify)
{
    if ( nRow < 0 || nRow >= GetRowCount())
        return FALSE;
...
BOOL CGridCtrl::BeginEditting()
{
	return OnEditCell(m_idCurrentCell.row, m_idCurrentCell.col, VK_SPACE);
}

BOOL CGridCtrl::EndEditting(BOOL bValidate)
{
	if (IsWindow(m_hCurEdit))
	{
		WPARAM	wParam;
		if (bValidate)	wParam = 1;
		else			wParam = 0;

		if (::SendMessage(m_hCurEdit, GVM_END_EDIT, wParam, 0) != 0)
			return FALSE;
	}
	return TRUE;
}
...
int CGridCtrl::FindRow(int iCol, CString text, int iStart, int iEnd)
{
	if (iCol < GetFixedColumnCount() ||
		iStart < GetFixedRowCount() || 
		iEnd < GetFixedRowCount() ||
		iStart >= GetRowCount() ||
		iEnd >= GetRowCount())
		return -1;
	...
	if (iStart <= iEnd)
	{
		for (row = iStart; row <= iEnd; row++)
			if (GetItemText(row, iCol) == text)
				return row;
	}
	else
	{
		for (row = iEnd; row >= iStart; row--)
			if (GetItemText(row, iCol) == text)
				return row;
	}
...
void CGridCtrl::SetEditMode(UINT nMode) 
{
	if (m_nEditMode == nMode)
		return;
	m_nEditMode = nMode;

	if (nMode != GVEM_NORMAL_GRID)
	{
		SetListMode(TRUE);
...
long CGridCtrl::GetVirtualWidth() const
{
    ...
    for (int i = 0; i < iColCount; i++)
        lVirtualWidth += m_arColProps[i].width;
...
int CGridCtrl::GetColumnWidth(int nCol) const
{
    ...
    return m_arColProps[nCol].width;
}

BOOL CGridCtrl::SetColumnWidth(int col, int width)
{
    ASSERT(col >= 0 && col < m_nCols && width >= 0);
    if (col < 0 || col >= m_nCols || width < 0) return FALSE;

    m_arColProps[col].width = width;

	// Resizing InPlaceEdit, if exists
	if (m_idCurrentCell.col == col)
		ResizeEditControl();

    return TRUE;
}

DWORD CGridCtrl::GetColumnStyle(int nCol) const
{
    ASSERT(nCol >= 0 && nCol < m_nCols);
    if (nCol < 0 || nCol >= m_nCols) return 0;

    return m_arColProps[nCol].style;
}

UINT CGridCtrl::GetColumnEdit(int nCol) const
{
    ASSERT(nCol >= 0 && nCol < m_nCols);
    if (nCol < 0 || nCol >= m_nCols) return 0;

    return m_arColProps[nCol].edit;
}

BOOL CGridCtrl::SetColumnEditStyle(int col, UINT nEdit, DWORD style)
{
	ASSERT(col >= 0 && col < m_nCols);
	if (col < 0 || col >= m_nCols) return FALSE;

	m_arColProps[col].edit = nEdit;
	m_arColProps[col].style = style;

	return TRUE;
}

CString CGridCtrl::GetColumnProp(int nCol, UINT nProp) const
{
    ASSERT(nCol >= 0 && nCol < m_nCols);
    if (nCol < 0 || nCol >= m_nCols) return "";

	switch( nProp ) {
	case GVCV_INITIAL:	return m_arColProps[nCol].initial;
	case GVCV_MASK:		return m_arColProps[nCol].mask;
	case GVCV_VALUES:	return m_arColProps[nCol].values;
	case GVCV_CBXON:	return m_arColProps[nCol].cbxOn;
	case GVCV_CBXOFF:	return m_arColProps[nCol].cbxOff;
	case GVCV_SRCSQL:	return m_arColProps[nCol].srcSQL;
	case GVCV_SRCLIST:	return m_arColProps[nCol].srcList;
	case GVCV_SRCFIELD:	return m_arColProps[nCol].srcField;
	}
    return "";
}

BOOL CGridCtrl::SetColumnProp(int col, UINT nProp, CString value)
{
    ASSERT(col >= 0 && col < m_nCols);
    if (col < 0 || col >= m_nCols) return FALSE;

	switch( nProp ) {
	case GVCV_INITIAL:	m_arColProps[col].initial	= value; break;
	case GVCV_MASK:		m_arColProps[col].mask		= value; break;
	case GVCV_CBXON:	m_arColProps[col].cbxOn		= value; break;
	case GVCV_CBXOFF:	m_arColProps[col].cbxOff	= value; break;
	case GVCV_SRCSQL:	m_arColProps[col].srcSQL	= value; break;
	case GVCV_SRCLIST:	m_arColProps[col].srcList	= value; break;
	case GVCV_SRCFIELD:	m_arColProps[col].srcField	= value; break;
	case GVCV_VALUES:
		{
			m_arColProps[col].values	= value;
			// Удаление карты значений
			if (value.IsEmpty() &&
				m_arColProps[col].mapVal != NULL)
			{
				delete m_arColProps[col].mapVal;
			...
			// Заполнение карты значений
			m_arColProps[col].mapVal = new CMapStringToString;
			CString	tmp,val,key;
			int		i,n,m,nLen = value.GetLength();
			...
			for (n = 0, m = -1; n < nLen; n = m + 1) {
				...
				// Добавление строки
				key.MakeLower();
				m_arColProps[col].mapVal->SetAt(key, val);
			}
		}
		break;
	}
...
UINT CGridCtrl::GetColumnType(int nCol) const
{
    ASSERT(nCol >= 0 && nCol < m_nCols);
    if (nCol < 0 || nCol >= m_nCols) return 0;

    return m_arColProps[nCol].type;
}

BOOL CGridCtrl::SetColumnType(int col, UINT nType) 
{
    ASSERT(col >= 0 && col < m_nCols);
    if (col < 0 || col >= m_nCols) return FALSE;

	m_arColProps[col].type = nType;

    return TRUE;
}

UINT CGridCtrl::GetColumnMaskType(int nCol) const
{
    ASSERT(nCol >= 0 && nCol < m_nCols);
    if (nCol < 0 || nCol >= m_nCols) return 0;

    return m_arColProps[nCol].maskType;
}

BOOL CGridCtrl::SetColumnMaskType(int col, UINT nMaskType, 
                                  UINT nLen, UINT nDec, 
                                  BOOL bSigned) 
{
    ASSERT(col >= 0 && col < m_nCols);
	...
	m_arColProps[col].maskType	= nMaskType;
	m_arColProps[col].len		= nLen;
	m_arColProps[col].dec		= nDec;
	m_arColProps[col].isSigned	= bSigned;
...
BOOL CGridCtrl::AutoSizeColumn(int nCol)
{
    ...
    m_arColProps[nCol].width = nWidth;

	// Resizing InPlaceEdit, if exists
	if (m_idCurrentCell.col == nCol)
		ResizeEditControl();
...
void CGridCtrl::AutoSize()
{
    ...
    for (int nCol = 0; nCol < nNumColumns; nCol++) {
        m_arColProps[nCol].width = 0;
		m_arColProps[nCol].edit = GVCE_EDIT;
	}
    ...
    for (int nCol = 0; nCol < nNumColumns; nCol++)
        ...
        {
            size = GetCellExtent(nRow, nCol, pDC);
            if (size.cx > (int) m_arColProps[nCol].width)
				m_arColProps[nCol].width = size.cx;
...
void CGridCtrl::ExpandColumnsToFit()
{
    ...
    int virtualWidth = (int) GetVirtualWidth();
    int nDifference = rect.Width() - virtualWidth;

    if (nDifference > 0)
	{
		for (int i = 0; i < GetColumnCount(); i++)
			m_arColProps[i].width += nDifference * m_arColProps[i].width / virtualWidth;
	}
	else
	{
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	{
		if (m_nEditMode == GVEM_NORMAL_GRID)
        ...
        return;
			 ...
			 m_MouseMode != MOUSE_OVER_ROW_DIVIDE)
    {
		if (!EndEditting()) return;

		if (!SetFocusCell(max(m_LeftClickDownCell.row, m_nFixedRows),
...
{
	// Send Message to the parent window
    SendMessageToParent(NM_RCLICK, 0, 0, "");
...
CSize CGridCtrl::GetTextExtent(LPCTSTR str, BOOL bUseSelectedFont)
{
    CDC* pDC = GetDC();
	...
	// Увеличить высоту текста по числу строк
	CString	txt(str);
	int		y = size.cy;
	int		i = -1;
	while ((i = txt.Find("\n", i + 1)) > 0)
...
CSize CGridCtrl::GetCellExtent(int nRow, int nCol, CDC* pDC)
{
	...
	CString	txt = GetItemText(nRow, nCol);
...
BOOL CGridCtrl::DrawCell(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk)
{
	...
	CString text(Item.szText);
	if (m_arColProps[nCol].edit == GVCE_LISTBOX &&
		m_arColProps[nCol].mapVal != NULL)
	{
		text.MakeLower();
		if (!m_arColProps[nCol].mapVal->Lookup(text, text))
			text = CString(Item.szText);
	}
    pDC->DrawText(text, rect, Item.nFormat | DT_NOPREFIX);
...
BOOL CGridCtrl::OnEditCell(int nRow, int nCol, UINT nChar)
{
	if (!EndEditting()) return FALSE;
    ...
    CRect rect;
    if (!GetCellRect(cell, rect)) FALSE;
    ...
    SendMessageToParent(GVN_BEGINLABELEDIT, nRow, nCol, "");
    ...
    CreateInPlaceEditControl(rect, dwStyle, nRow, nCol, Item.szText, nChar);
...
void CGridCtrl::CreateInPlaceEditControl(CRect& rect, DWORD dwStyle, 
                                           int nRow, int nCol, 
										   LPCTSTR szText, int nChar)
{
	...
	CString	text = CString(szText);
	DWORD	style = m_arColProps[nCol].style;

	switch (GetColumnEdit(nCol)) {
	case GVCE_NONE:
		return;
	case GVCE_EDIT:
	{
		// InPlaceEdit Control
		if (style & GVCS_MULTILINE)
			dwStyle |= ES_MULTILINE|
			ES_AUTOHSCROLL|ES_AUTOVSCROLL;
		// Создание объекта
		CInPlaceEdit* pEdit;
		pEdit = new CInPlaceEdit(this, rect, dwStyle, 
				IDC_INPLACE_CONTROL, nRow, nCol, 
				nChar, text, m_arColProps[nCol]);
		m_hCurEdit = pEdit->m_hWnd;
		m_pCurEdit = (CWnd* )pEdit;
		break;
	}
	case GVCE_CHECKBOX:
		break;
	case GVCE_LISTBOX:
	{
		...
		text.MakeLower();
		if (m_arColProps[nCol].mapVal != NULL)
			m_arColProps[nCol].mapVal->Lookup(text, text);
		// Создание объекта
		CInPlaceList* pList;
		pList = new CInPlaceList(this, rect, 
			dwStyle, IDC_INPLACE_CONTROL, 
			nRow, nCol, nChar, text, 
			m_arColProps[nCol].values);
		m_hCurEdit = pList->m_hWnd;
		m_pCurEdit = (CWnd* )pList;
		break;
	}
	case GVCE_LOOKUPLIST:
		break;
	case GVCE_POPUPLIST:
		break;
	}
